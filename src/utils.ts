...
export const deserialize_user_token = async (acc_info: AccountInfo<Buffer>) => {
  const account_data: UserAddresTokenMint = deserialize(
    UserAddresTokenMintSchema,
    UserAddresTokenMint,
    acc_info.data
  );
...
export const deserialize_proposal = async (acc_info: AccountInfo<Buffer>) => {
  const account_data: Proposal = deserialize(
    ProposalSchema,
    Proposal,
    acc_info.data
  );
...
export const deserialize_fundraising_data = async (data: Buffer) => {
  const account_data: FundRaising = deserialize(
    FundRaisingSchema,
    FundRaising,
    data
  );
...
export const deserialize_nftterms_data = async (data: Buffer) => {
  ...
  const token_mint = new PublicKey(
    bs58.encode(account_data.tokenization_mint).toString()
  );
  ...
  console.log(
    "lamports_per_token_buyout " + account_data.lamports_per_token_buyout
  );

  return account_data;
};
export const get_ata = async (
  owner: PublicKey,
  mint: PublicKey,
  onCurve: boolean,
...
export const get_fundraising_account = async (nft_mint: PublicKey) => {
  const pda = PublicKey.findProgramAddressSync(
    [Buffer.from("fund"), nft_mint.toBytes()],
    programID
  );
...
export const get_vote_account_pda = async (tokenezation: PublicKey) => {
  const pda = PublicKey.findProgramAddressSync(
    [Buffer.from("vote"), tokenezation.toBytes()],
    programID
  );

  return pda;
};
export const get_voter_pda = async (
  wallet: WalletContextState,
  token_mint: PublicKey
) => {
  const pda = PublicKey.findProgramAddressSync(
    [wallet.publicKey!.toBytes(), token_mint.toBytes()],
    programID
  );
...
export const get_registered_nft_account_address = async (
  nft_mint: PublicKey
) => {
